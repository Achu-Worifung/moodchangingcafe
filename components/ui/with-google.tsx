...
import { doWithGoogle } from "@/lib/auth";
import { toast } from "sonner";
import { useRouter } from "next/navigation";
export default function WithGoogle({signIn = true}: {signIn?: boolean}) {
    const router = useRouter();
  return (
    <Button
      variant={"outline"}
      className="w-full flex items-center justify-center gap-2 outline-2"
      onClick={async () => {
        try {
          await doWithGoogle();
          toast.success(signIn ? "Login successful!" : "Account created successfully!");
          setTimeout(() => {
            router.push("/");
          }, 1000);
        } catch (error) {
